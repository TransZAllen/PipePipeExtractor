...
//        }
    ...
    public List<SubtitlesStream> getSubtitles(final MediaFormat format) throws ParsingException {
        String logMessage = "format=" + format;
        ...
        LogUtil.logWithMessage("tree-test02", "Total number of caption tracks: " + captionsArray.size());
        ...
        for (int i = 0; i < captionsArray.size(); i++) {
            ...
            LogUtil.logWithMessage("tree-test02", "Language: " + languageCode + ", Base URL: " + baseUrl + ", VSS ID: " + vssId);
            ...
            if (languageCode != null && baseUrl != null && vssId != null) {
                ...
                LogUtil.logWithMessage("tree-test02", logMessage_01);
                logMessage_01 = cleanUrl;
                LogUtil.logWithMessage("tree-test02", logMessage_01);

                String subtitleUrl = cleanUrl + "&fmt=" + format.getSuffix();

                String localSubtitleUrl = "";
                try {
                    localSubtitleUrl = SubtitleDeduplicator.process(subtitleUrl, format);
                    LogUtil.logWithMessage("tree-test02", "localSubtitleUrl=" + localSubtitleUrl);
                } catch (Exception e) {
                    // TODO: handle exception
                    logMessage_01 = "localSubtitleUrl is not OK." + "e=" + e;
                    LogUtil.logWithMessage("tree-test02", logMessage_01);
                }
                if ( new File(localSubtitleUrl).exists() ) {
                    LogUtil.logWithMessage("tree-test02", "localSubtitleUrl exist");
                } else {
                    LogUtil.logWithMessage("tree-test02", "localSubtitleUrl Not exist");
                }

                subtitlesToReturn.add(new SubtitlesStream.Builder()
                        //.setContent(cleanUrl + "&fmt=" + format.getSuffix(), true)
                        .setContent(localSubtitleUrl, true) // ✅ 设置为本地路径
                        .setMediaFormat(format)
                        // 这是远程链接
                        //.setContent(vttUrl, true)
                        // 明确告诉 ExoPlayer：这是 VTT 格式
                        //.setMediaFormat("text/vtt")
        ...
        logMessage = "Total subtitles found: " + subtitlesToReturn.size();
        LogUtil.logWithMessage("tree-test02", logMessage);
