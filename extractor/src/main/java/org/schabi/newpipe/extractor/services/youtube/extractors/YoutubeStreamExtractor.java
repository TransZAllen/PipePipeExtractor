...
//        }
    ...
    public List<SubtitlesStream> getSubtitles(final MediaFormat format) throws ParsingException {
        ...
        for (int i = 0; i < captionsArray.size(); i++) {
            ...
            if (languageCode != null && baseUrl != null && vssId != null) {
                ...
                subtitlesToReturn.add(new SubtitlesStream.Builder()
                        ...
                        .setRemoteUrl(remoteSubtitleUrl)
                        .build());
        ...
        // There are three types of subtitles on YouTube:
        //
        //   a) Uploaded subtitles (provided by the video author).
        //   b) Auto-generated subtitles (produced automatically by YouTube).
        //   c) Auto-translated subtitles (generated by translating either a) or b)).
        //
        // Auto-translated subtitles always require an existing subtitle
        // as the translation source. The translation quality depends on
        // the source subtitle type. Uploaded subtitles usually provide
        // the best quality, so we prefer them when available.

        translatableSubtitle = buildAutoTranslateSubtitleWithSourceUrl(subtitlesToReturn);
    ...
    // 1) Select a better subtitle being source to auto-translate
    //    a destination language, and get the index of the selected subtitle;
    // 2) Build a new subtitle stream as 'auto-translate',
    //    using the remote subtitle url of the selected subtitle.
    private SubtitlesStream buildAutoTranslateSubtitleWithSourceUrl(
                                    List<SubtitlesStream> subtitlesList) {
        if (false == hasAnySourceSubtitle(subtitlesList)) {
            return null;
        }

        int selectedSourceID = findBestSourceSubtitle(subtitlesList);

        SubtitlesStream selectedSubtitle = subtitlesList.get(selectedSourceID);

        SubtitlesStream subtitleToAutoTranslate = buildAutoTranslateSubtitleStream(
                                                        selectedSubtitle);
        return subtitleToAutoTranslate;
    }

    private int totalSourceSubtitles(List<SubtitlesStream> subtitles) {
        int totalSubtitles = subtitles.size();
        return totalSubtitles;
    }

    private boolean hasAnySourceSubtitle(List<SubtitlesStream> subtitlesList) {
    ...
    private int findBestSourceSubtitle(List<SubtitlesStream> subtitles) {
        int subtitleID = preferUploadedSubtitle(subtitles);
        return subtitleID;
    }

    // Prefer uploaded subtitle (higher quality).
    // If none found, fallback to the auto-generated subtitle.
    private int preferUploadedSubtitle(List<SubtitlesStream> subtitles) {
        int i = 0;
        int i_AutoGenerated = 0;
        int totalSource = totalSourceSubtitles(subtitles);

        for (i = 0; i < (totalSource); i++) {
            SubtitlesStream i_subtitle = subtitles.get(i);
            if (false == i_subtitle.isAutoGenerated()) {
                return i;
            } else {
                i_AutoGenerated = i;
            }
        ...
        // There is no uploaded subtitle.
        return i_AutoGenerated;
    }

    private SubtitlesStream buildAutoTranslateSubtitleStream(
                                    SubtitlesStream selectedSource) {
        String selectedSourceUrl = selectedSource.getRemoteUrl();

        SubtitlesStream newSubtitleStream = null;
        newSubtitleStream = useRemoteUrlToAutoTranslate(
                                            selectedSource,
                                            selectedSourceUrl);
        return newSubtitleStream;
    }
